...
const INC=2000,WAIT=30000,ANIM=460;
const $=id=>document.getElementById(id);

function makeBoard(){
  board=Array.from({length:8},()=>Array(8).fill(null));
  for(let r=0;r<3;r++)for(let c=0;c<8;c++)if((r+c)%2===1)
    board[r][c]={color:"black",king:false};
  for(let r=5;r<8;r++)for(let c=0;c<8;c++)if((r+c)%2===1)
    board[r][c]={color:"white",king:false};
}
function newGame(){
  clearTimeout(botTimer);
  makeBoard();
  turn="white";selected=null;legal=[];gameOver=false;
...
function startClock(){
  ...
  timerHandle=setInterval(()=>{
    ...
    if(turn==="white")whiteMs=Math.max(0,whiteMs-dt);
...
function updateClock(){
  const f=ms=>{
    let s=Math.max(0,Math.ceil(ms/1000)),m=Math.floor(s/60),q=s%60;
...
function botMove(){
  ...
  animateMove(mv,()=>{
    board=LerakRules.apply(board,mv);
    turn="white";selected=null;legal=[];lastTick=performance.now();
    render();afterMove();
  });
}
function render(){
  ...
  $("status").textContent=gameOver?"ការប្រកួតបានបញ្ចប់":
    (turn==="white"?"វេនរបស់អ្នក":"វេន Bot 🤖");
  updateClock();
}
function animateMove(move,done){
  const b=$("board");
  const [fr,fc]=move.from,[tr,tc]=move.to;
  const fromCell=b.querySelector(`[data-r="${fr}"][data-c="${fc}"]`);
  const toCell=b.querySelector(`[data-r="${tr}"][data-c="${tc}"]`);
  if(!fromCell||!toCell){done();return}
  const piece=fromCell.querySelector(".piece");
  if(!piece){done();return}
  const a=fromCell.getBoundingClientRect(),z=toCell.getBoundingClientRect();
  const dx=z.left-a.left,dy=z.top-a.top;
  const captured=move.over
    ?b.querySelector(`[data-r="${move.over[0]}"][data-c="${move.over[1]}"] .piece`)
    :null;
  piece.classList.add("moving");
  piece.style.setProperty("--move-x",`${dx}px`);
  piece.style.setProperty("--move-y",`${dy}px`);
...
function clickCell(r,c){
  if(gameOver||turn!=="white"||$("board").classList.contains("animating"))return;
  ...
  if(chosen){
    ...
    animateMove(chosen,()=>{
      ...
      selected=null;legal=[];turn="black";lastTick=performance.now();
      render();afterMove();
    });
    return;
  }
...
function afterMove(){
  if(!LerakRules.hasAny(board,turn)){
    finish(turn==="white"?"black":"white","មិនអាចចលនា");return;
  }
...
function showRules(){
  ...
  • ភាគីសចាប់ផ្តើម<br>
  • មួយវេនដើរ 1 កូន<br>
  • ត្រូវចាប់កូន ប្រសិនបើមានការចាប់<br>
  • អាចចាប់បន្តតាមច្បាប់ចាប់របស់ហ្គេម<br>
  • ឡើងដល់ចុងក្តារ → ក្លាយជា King<br>
  • ពេលលេង 3:00 + 2 វិនាទីក្នុងមួយចលនា<br>
  • រង់ចាំ 30 វិនាទីសម្រាប់គូប្រកួត; បើគ្មាន Bot ចូលជំនួស
  </p>`;
