...
(function () {
  ...
  function statusText(status){
    if(status === "approved") return '<span class="mh-status approved">✅ បានបញ្ចូល</span>';
    if(status === "rejected") return '<span class="mh-status rejected">❌ បដិសេធ</span>';
  ...
  function render(docs){
    ...
    if(!docs.length){
      box.innerHTML = '<div class="mh-empty"><div class="mh-empty-icon">📭</div><b>មិនទាន់មានសារ</b><small>ប្រវត្តិ Top Up របស់អ្នកនឹងបង្ហាញនៅទីនេះ</small></div>';
      return;
    }

    box.innerHTML = docs.map(doc => {
      const d = doc.data();
      return `<article class="mh-item ${d.status || "pending"}">
        ...
        <div class="mh-content">
          ...
          <div class="mh-meta">${statusText(d.status)}<span>🕐 ${dateText(d.createdAt)}</span></div>
          <small class="mh-package">${d.packageId || "Top Up"} · ID: ${doc.id}</small>
        </div>
      </article>`;
    }).join("");
  }
    ...
    unsubscribe = firebase.firestore()
      ...
      .onSnapshot(
        ...
        err => {
          ...
          box.innerHTML = '<div class="mh-empty mh-error">❌ មិនអាចអានសារបាន។ សូម Refresh។</div>';
        }
      );
  }
  ...
  document.addEventListener("click",function(e){
    const modal = $("messagesModal");
    if(modal && e.target === modal) window.closeMessagesHistory();
  });
  ...
  function init(){
    ...
    firebase.auth().onAuthStateChanged(user => {
      if(user) load(user);
        ...
        if(unsubscribe) unsubscribe();
        const box = $("messagesHistory");
        if(box) box.innerHTML = '<div class="mh-empty"><div class="mh-empty-icon">🔐</div><b>សូមចូលគណនី</b><small>Login ដើម្បីមើលប្រវត្តិ Top Up</small></div>';
        const badge = $("messagesBadge");
