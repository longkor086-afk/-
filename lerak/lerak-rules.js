/* Lerak rule helpers.
   Board: 8x8. White moves first. Men move diagonally forward.
   Captures are mandatory. Multiple captures are supported.
   Reaching the last row promotes to king. Kings move/capture diagonally
   one square at a time. This standalone build is intentionally independent
   of the old repository game.js. */
(function(){
  const DIRS={white:[[1,-1],[1,1]],black:[[-1,-1],[-1,1]],king:[[1,-1],[1,1],[-1,-1],[-1,1]]};
  window.LerakRules={
    ...
    moves(board,color){
      const caps=this.captures(board,color); if(caps.length)return caps;
      const out=[];
      for(let r=0;r<8;r++)for(let c=0;c<8;c++){
        const p=board[r][c]; if(!p||p.color!==color)continue;
        for(const [dr,dc] of this.dirs(p)){
