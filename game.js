  ...
  if(selected){
    ...
    if(legal){
      ...
      if(cap.victims.length===2 && surround.removed>0)
        ...
        message(`កូនទើបដើររួច។ បើចង់ហៅរែក សូមចុច «🪤 ហៅរែក»។ បើមិនហៅ សូមចុចក្រឡាផ្សេងដើម្បីបន្តវេន។`);
...
function startCallFromPending(){
  ...
  if(captures.length===0){
    ...
    selected=null;
    turn=enemy;
    message("ហៅរែកអាសាបង់។ ប៊ូតុងហៅរែកបិទ ហើយគូប្រកួតអាចដើរកូនណាក៏បាន ឬប្រើយុទ្ធសាស្ត្រផ្សេង។");
    render();
    return;
  }
...
function render(){
  ...
  // មានតែប៊ូតុង «ហៅរែក» ប៉ុណ្ណោះ។
  // ប៊ូតុងបង្ហាញតែនៅពេលវេនរបស់អ្នក និងមិនកំពុងរង់ចាំ
  // គូប្រកួតឆ្លើយតបការហៅរែក។
  let oldCall=document.getElementById("callTrapButton");
  if(oldCall)oldCall.remove();

  if(phase==="normal" && !callTrap && (pendingCall || callArmed)){
    const btn=document.createElement("button");
