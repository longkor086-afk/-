...
function clickNormal(r,c){
  ...
  if(pendingCall){
    message("កំពុងដំណើរការហៅរែកសម្រាប់ក្រឡាគោលដៅ។");
    return;
  }
  ...
  if(selected){
    ...
    if(legal){
      ...
      // ហៅរែកគឺជាជម្រើស មិនមែនជាកាតព្វកិច្ច។
      // មានតែពេលអ្នកបានចុច «ហៅរែក» មុនដើរ
      // ទើបក្រឡាថ្មីនេះត្រូវយកទៅពិនិត្យហៅរែក។
      if(callArmed){
      ...
      // មិនបានចុចហៅរែក -> ដើរធម្មតា និងប្តូរវេនភ្លាម។
      pendingCall=null;
      turn=turn===W?B:W;
      ...
      if(cap.victims.length===2 && surround.removed>0)
        ...
        message(`ដើរធម្មតារួច។ វេន ${sideName(turn)}។`);
...
function render(){
  ...
  if(phase==="normal" && !callTrap && !pendingCall){
    ...
    btn.textContent=callArmed ? "🪤 ហៅរែក (បានបើក — ដើរកូនគោលដៅ)" : "🪤 ហៅរែក";
    btn.style.display="block";
    btn.style.margin="10px auto";
  ...
  turnEl.textContent=gameOver?"ចប់ការប្រកួត":
    callTrap
      ...
      : pendingCall
        ? `🪤 កំពុងពិនិត្យហៅរែក`
        : callArmed
          ? `វេន៖ ${sideName(turn)} · ហៅរែកបានបើក — ដើរកូនគោលដៅ`
