...
let pendingCall=null; // only the piece just moved can be called
let callArmed=false; // button pressed before moving
...
function clickNormal(r,c){
  ...
  if(pendingCall){
    message("កូនទើបដើររួច។ សូមជ្រើស «🪤 ហៅរែក» ឬ «បន្ត (មិនហៅរែក)» មុនប្តូរវេន។");
    return;
  }
  ...
  if(selected){
    ...
    if(legal){
      ...
      // រក្សាក្រឡាដែលទើបដើរ ដើម្បីឱ្យអ្នកសម្រេចហៅរែកដោយដៃ។
      pendingCall={side:moved.side,r,c};
      ...
      if(cap.victims.length===2 && surround.removed>0)
        message(`រែកបាន ២ កូន និងព័ទ្ធបាន ${kh(surround.removed)} កូន។ បើចង់ហៅរែក សូមចុចប៊ូតុង។`);
      else if(cap.victims.length===2)
        message(`រែកបាន ២ កូន។ បើចង់ហៅរែក សូមចុចប៊ូតុង។`);
      else if(surround.removed>0)
        message(`ព័ទ្ធបាន ${kh(surround.removed)} កូន។ បើចង់ហៅរែក សូមចុចប៊ូតុង។`);
      else
        message(`កូនទើបដើររួច។ បើចង់ហៅរែក ត្រូវចុច «🪤 ហៅរែក»។`);
...
function tryCallTrap(){
  ...
  // ចុចមុនដើរ = បើកមុខងារ; មិនទាន់ហៅរែក
  if(!pendingCall){
    callArmed=true;
    selected=null;
    message("🪤 បានបើកមុខងារហៅរែក។ ឥឡូវសូមដើរកូនទ័ពគោលដៅសិន។");
    render();
    return;
  }
  startCallFromPending();
...
function startCallFromPending(){
  if(!pendingCall || callTrap)return;
  const {r,c,side}=pendingCall;
  const p=board[r][c];
  if(!p || p.side!==side || p.king){
    message("ហៅរែកបានតែកូនទ័ពដែលទើបដើរចូលក្រឡានេះប៉ុណ្ណោះ។");
    return;
  }
  ...
  if(captures.length===0){
    // ហៅរែកអាសាបង់។ គូប្រកួតបានវេនធម្មតា
    // ហើយអាចដើរកូនណាក៏បាន ឬប្រើយុទ្ធសាស្ត្រផ្សេង។
    pendingCall=null;
    callArmed=false;
    selected=null;
    turn=enemy;
    message("ហៅរែកអាសាបង់។ គូប្រកួតអាចដើរកូនណាក៏បាន ឬបន្តយុទ្ធសាស្ត្ររបស់ខ្លួនតាមធម្មតា។");
    render();
    return;
  }
  ...
  selected=null;
  turn=enemy;
  message(`🪤 ហៅរែក! ${sideName(turn)} អាចជ្រើសកូនណាមួយក្នុង ${kh(captures.length)} ជម្រើស ដើម្បីមករែក។`);
  render();
}

function skipCallTrap(){
  if(!pendingCall || callTrap)return;
  pendingCall=null;
  callArmed=false;
  selected=null;
  turn=turn===W?B:W;
  message(`មិនហៅរែក។ ឥឡូវ ${sideName(turn)} ដើរ។`);
  render();
}
...
function render(){
  ...
  // ប៊ូតុងហៅរែកបង្ហាញតែបន្ទាប់ពីដើរកូនទ័ពរួច
  let oldCall=document.getElementById("callTrapButton");
  if(oldCall)oldCall.remove();
  let oldSkip=document.getElementById("skipCallButton");
  if(oldSkip)oldSkip.remove();

  if(phase==="normal" && !callTrap){
    const btn=document.createElement("button");
    btn.id="callTrapButton";
    btn.textContent=callArmed ? "🪤 ហៅរែក (បានបើក — ដើរកូនទ័ព)" : "🪤 ហៅរែក";
    btn.style.display="block";
    btn.style.margin="10px auto 6px";
    btn.onclick=tryCallTrap;
    boardEl.after(btn);

    if(pendingCall){
      const skip=document.createElement("button");
      skip.id="skipCallButton";
      skip.textContent="បន្ត (មិនហៅរែក)";
      skip.style.display="block";
      skip.style.margin="0 auto 10px";
      skip.onclick=skipCallTrap;
      btn.after(skip);
    }
  }
  ...
  turnEl.textContent=gameOver?"ចប់ការប្រកួត":
    callTrap
      ...
      : pendingCall
        ? `វេន៖ ${sideName(pendingCall.side)} · សម្រេចហៅរែក`
        : callArmed
          ? `វេន៖ ${sideName(turn)} · ហៅរែកបានបើក — ដើរកូនទ័ពគោលដៅ`
